...
import codecs
import csv
import re #re:文字列の正規表現する際に使用するモジュール
import MeCab
import os
    ...
    try:
        for sentense in sentenses: # 文の数だけ繰り返す
            ...
            for word in result_words:
                try:
                    word_toarray = re.split('[\t,]', word)#正規表現'[\t,]'で空白(tab)または,で分割
                    if word_toarray[7] in negaposi_dic:
                        negaposi = int(negaposi_dic[word_toarray[7]])  # その文のネガポジ
                        print(word_toarray[7],negaposi_dic[word_toarray[7]], flush=True) # 評価リストに入っていたワードとその評価 、printの出力結果をflush = trueで即時表示
                except Exception as e:
                    pass #辞書にない単語が出てきた時は無視
                    #print('%rなんでだよお' % e, flush=True)
